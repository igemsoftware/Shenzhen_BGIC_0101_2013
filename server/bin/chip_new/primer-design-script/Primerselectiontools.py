...
def evaluateOverlaps(overlapseqs, overlaptemps, deltaGThreshold, selfDimersThreshold):
    ...
    for pl in poslees:
        for ll in range(lengthleeway+1):
            ...
            for coord in testseqcoords:
                ...
                #print overlapseqs
            finaloverlap = evaluateOverlaps(overlapseqs, overlaptemps, deltaGThreshold, selfDimersThreshold)
            if finaloverlap:
                return finaloverlap[1]
        #print "position change"

#this needs to be fixed;      
def optimizedSplit(seq, oligosizemax, lengthleeway, positionleeway, avgoverlapsize, overlaptemps, deltaGThreshold, selfDimersThreshold, num_of_oligos=-1):
    seqsize = len(seq)
    #print seqsize
    ...
    while True:
        ...
        if (newoptov[0]+oligosizemax) >= len(seq):
            ...
            lengthoffset = ((newoptov[1]-newoptov[0])-avgoverlapsize)*-1
    ...
    if len(oligos)>num_of_oligos:
        oligos = optimizedSplit(seq, oligosizemax, lengthleeway, positionleeway, avgoverlapsize, overlaptemps, deltaGThreshold, selfDimersThreshold, num_of_oligos)
...
def primerdimers(primer1, primer2):
    ...
    for i in range(len(primer1)):
        score.append(complementcompare(primer1[0:i+1], primer2[len(primer1)-i-1:len(primer1)]))
    #print score
    return max(score)
...
# based on origin of replication and terminus locations, returns which strand to target for any particular sequence feature on MG1655
def strandChooser(thefeature):
    ...
    featurestrand = thefeature.strand

    if featuremiddle > repstart or featuremiddle < repend:
        # Replichore 1: want to target +strand, so return -strand because it is complementary
        return -1
    else:
        # Replichore 2: want to target -strand, so return +strand because it is complementary
        return 1
