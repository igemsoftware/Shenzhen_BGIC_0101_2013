...
def evaluateOverlaps(seq, overlapseqs, overlaptemps, deltaGThreshold, selfDimersThreshold, seqsToAvoid, coordsOfAvoidedRegionsInSeq):
    ...
    overlapseqs = avoidedSeqsInOverlapRegions(seq, overlapseqs, seqsToAvoid, coordsOfAvoidedRegionsInSeq)
    if len(overlapseqs) > 0:
        overlapseqs.sort(lambda x, y: cmp(abs(x[2]), abs(y[2])))
        finaloverlap = overlapseqs[0]
        return finaloverlap
    else:
        return []

def optimizeOverlap(overlap, seq, lengthleeway, positionleeway, overlaptemps, deltaGThreshold, selfDimersThreshold, seqsToAvoid, coordsOfAvoidedRegionsInSeq):
    #First Check Characteristics of all oligos at the same positions with different lengths
    
    newoverlap = overlap[:]
    poslees = range(positionleeway*-1,(positionleeway)+1)
    poslees.sort(lambda x, y: cmp(abs(x), abs(y)))
    #print poslees
    for pl in poslees:
        for ll in range(lengthleeway+1):
            ...
            for coord in testseqcoords:
                ...
                #print overlapseqs
            finaloverlap = evaluateOverlaps(seq, overlapseqs, overlaptemps, deltaGThreshold, selfDimersThreshold, seqsToAvoid, coordsOfAvoidedRegionsInSeq)
            if finaloverlap:
                return finaloverlap[1]
        #print "position change"

def optimizedSplit(seq, oligosizemax, lengthleeway, positionleeway, avgoverlapsize, overlaptemps, deltaGThreshold, selfDimersThreshold, seqsToAvoid, num_of_oligos=-1):
    # ensure that seqsToAvoid contains the reverse complementary sequences as well
    comps = []
    if seqsToAvoid != []:
        print "Avoiding forbidden sequences in overlap regions. Execution may take a while..."
    for seqToAvoid in seqsToAvoid:        
        # reverse complement
        complement = string.maketrans('ACGTacgt','TGCAtgca')
        c = (str(seqToAvoid).translate(complement))[::-1]
        if not c in seqsToAvoid:
            comps.append(c)
    for comp in comps:
        seqsToAvoid.append(comp)
    
    # find the coordinates to avoid
    coordsOfAvoidedRegionsInSeq = []
    for seqToAvoid in seqsToAvoid:
        s = copy.deepcopy(seq)
        l = len(str(seqToAvoid))
        numAvoidedRegions = s.count(seqToAvoid)
        offset = 0
        while s.find(str(seqToAvoid)) != -1:
            firstLoc = s.find(str(seqToAvoid))
            coordsOfAvoidedRegionsInSeq.append([offset + firstLoc, offset + firstLoc + l])
            s = s[firstLoc + l:]
            offset += firstLoc + l
    
    seqsize = len(seq)
    #print seqsize
    addedperoligo = oligosizemax-avgoverlapsize-lengthleeway/2
    #print addedperoligo
    if num_of_oligos==-1:
        num_of_oligos = (seqsize / addedperoligo)
    if not (seqsize % addedperoligo)==0:
        num_of_oligos += 1
    #print num_of_oligos

    #avgoligolength = ((seqsize) / num_of_oligos) + 1 - avgoverlapsize
    avgoligolength = ((seqsize - ((num_of_oligos+1)*avgoverlapsize))/num_of_oligos) + 1
    ...
    while True:
        newoptov = optimizeOverlap(optov, seq, lengthleeway, positionleeway, overlaptemps, deltaGThreshold, selfDimersThreshold, seqsToAvoid, coordsOfAvoidedRegionsInSeq)
    ...
    if len(oligos)>num_of_oligos:
        oligos = optimizedSplit(seq, oligosizemax, lengthleeway, positionleeway, avgoverlapsize, overlaptemps, deltaGThreshold, selfDimersThreshold, seqsToAvoid, num_of_oligos)
