...
for ($i = 0; $i < count($_FILES['uploadedfiles']['name']); $i++) {
    if (!file_exists("upload/" . $_FILES["uploadedfiles"]["name"][$i])) {
        move_uploaded_file($_FILES["uploadedfiles"]["tmp_name"][$i],
            "upload/" . $_FILES["uploadedfiles"]["name"][$i]);
    }
    $flg++;
}
...
//    error_log(print_r($datasetname, true));
    ...
    chdir('..');
    mkdir($outdir);
    copy('server/upload/'.$fasta, $outdir.$fasta);
    copy('server/upload/'.$gff, $outdir.$gff);
    $err = Array(); 
    exec("bin/prepare-refseqs.pl --fasta $outdir$fasta --out $outdir 2>&1", $err); 
    //error_log("1".print_r($err, true), 3, 'server/debug.log');
    exec("bin/biodb-to-json.pl --conf server/normal.json --out $outdir 2>&1", $err);
    ///error_log("2".print_r($err, true), 3, 'server/debug.log');
    exec("bin/add-json.pl '{ \"dataset_id\": \"$fasta\" }' {$outdir}trackList.json 2>&1", $err);
    ...
    header('Location: http://'.$_SERVER['SERVER_NAME']
            .'/Genovo/jbrowse/index.html?data='.$outdir);
